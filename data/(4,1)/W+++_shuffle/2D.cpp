...
// (4,1) 이동 규칙 (총 8방향)
int dx[8] = {4, 4, -4, -4, 1, 1, -1, -1};
int dy[8] = {1, -1, 1, -1, 4, -4, 4, -4};
int degree[500][500];
...
int main(int argc, char* argv[]) {
    ...
    std::mt19937 gen(rd());
    std::shuffle(a, a + 8, gen);
    {
        vector<int> v;v.resize(8);
        for(int i=0;i<8;i++)v[i]=dx[i];
        for(int i=0;i<8;i++)dx[i]=v[a[i]];
    }
    {
        vector<int> v;v.resize(8);
        for(int i=0;i<8;i++)v[i]=dy[i];
        for(int i=0;i<8;i++)dy[i]=v[a[i]];
    }
