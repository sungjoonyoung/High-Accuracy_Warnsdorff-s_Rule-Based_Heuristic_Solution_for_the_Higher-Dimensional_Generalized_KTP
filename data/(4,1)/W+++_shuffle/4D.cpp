...
int main(int argc, char* argv[]) {
    ...
    //random first
    int a[48];
    for(int i=0;i<48;i++)a[i]=i;
    std::random_device rd;
    std::mt19937 gen(rd());
    std::shuffle(a, a + 48, gen);
    {
        vector<int> v;v.resize(48);
        for(int i=0;i<48;i++)v[i]=dx[i];
        for(int i=0;i<48;i++)dx[i]=v[a[i]];
    }
    {
        vector<int> v;v.resize(48);
        for(int i=0;i<48;i++)v[i]=dy[i];
        for(int i=0;i<48;i++)dy[i]=v[a[i]];
    }
    {
        vector<int> v;v.resize(48);
        for(int i=0;i<48;i++)v[i]=dz[i];
        for(int i=0;i<48;i++)dz[i]=v[a[i]];
    }
    {
        vector<int> v;v.resize(48);
        for(int i=0;i<48;i++)v[i]=dw[i];
        for(int i=0;i<48;i++)dw[i]=v[a[i]];
    }
