...
file_pattern = r"C:\Users\sungs\OneDrive\Documents\2-1\Project\data\accu-more\data_degree+distance\4D\*.txt"
...
for filepath in glob.glob(file_pattern):
    ...
    for line in content.splitlines():
        ...
        # Start position + 실행시간 줄 처리 (4차원 좌표)
        m = re.search(
            r"Start position:\s*(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s*-\s*(\w+)\s*Execution time:\s*(\d+)\s*ms\s*\((\d+)\s*μs\)", line)
        if m:
            x, y, z, w, status, ms_time, us_time = m.groups()
    ...
    results.append({
        ...
        "평균 시간 (ms)": avg_ms,
        "평균 시간 (μs)": avg_us,
        "홀수 XYZW 합 성공률": odd_success_rate,
        "짝수 XYZW 합 성공률": even_success_rate
    })

df = pd.DataFrame(results)
df.to_excel("summary_by_file_odd_even_4D.xlsx", index=False)
print("완료! summary_by_file_odd_even_4D.xlsx 파일이 생성되었습니다.")
